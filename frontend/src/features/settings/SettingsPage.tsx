...
export function SettingsPage(): React.JSX.Element {
  ...
  const members = useQuery({
    ...
    enabled: Boolean(organisationId) && canManageMembers(role),
  });

  const diagnostics = useQuery({
    queryKey: queryKeys.diagnostics(organisationId ?? ""),
    queryFn: () => jobsApi.diagnostics(),
    enabled: Boolean(organisationId) && canViewDiagnostics(role),
  });
  ...
  const removeMember = useMutation({
    mutationFn: (userId: string) =>
      organisationsApi.removeMember(organisationId ?? "", userId),
    onSuccess: () => {
      setRemoving(null);
      invalidateMembers();
