...
class SQLiteMeetingRepository(AbstractMeetingRepository):
    ...
    def save(self, meeting: Meeting) -> None:
        with self._store.transaction() as connection:
            connection.execute(
                "INSERT INTO meetings(meeting_id, meeting_date, payload) VALUES (?, ?, ?) "
                "ON CONFLICT(meeting_id) DO UPDATE SET meeting_date=excluded.meeting_date, payload=excluded.payload",
                (meeting.meeting_id, meeting.meeting_date.isoformat(), _dump(meeting)),
            )
...
class SQLiteExtractionRepository(AbstractExtractionRepository):
    ...
    def save(self, result: ExtractionResult) -> None:
        with self._store.transaction() as connection:
            connection.execute(
                "INSERT INTO extraction_results(meeting_id, extracted_at, payload) VALUES (?, ?, ?) "
                "ON CONFLICT(meeting_id) DO UPDATE SET extracted_at=excluded.extracted_at, payload=excluded.payload",
                (result.meeting_id, result.extracted_at.isoformat(), _dump(result)),
            )
...
class SQLiteMentionRepository(AbstractMentionRepository):
    ...
    def create(self, mention: EntityMention) -> None:
        with self._store.transaction() as connection:
            connection.execute(
                "INSERT INTO entity_mentions(mention_id, meeting_id, entity_id, entity_type, payload) VALUES (?, ?, ?, ?, ?) "
                "ON CONFLICT(mention_id) DO UPDATE SET meeting_id=excluded.meeting_id, entity_id=excluded.entity_id, entity_type=excluded.entity_type, payload=excluded.payload",
                (mention.mention_id, mention.meeting_id, mention.entity_id, mention.entity_type.value, _dump(mention)),
            )
...
class SQLiteDependencyRepository(AbstractDependencyRepository):
    ...
    def save(self, dependency: ExplicitDependency) -> None:
        with self._store.transaction() as connection:
            connection.execute(
                "INSERT INTO dependencies(dependency_id, source_entity_id, target_entity_id, meeting_id, relationship_type, payload) VALUES (?, ?, ?, ?, ?, ?) "
                "ON CONFLICT(dependency_id) DO UPDATE SET source_entity_id=excluded.source_entity_id, target_entity_id=excluded.target_entity_id, meeting_id=excluded.meeting_id, relationship_type=excluded.relationship_type, payload=excluded.payload",
                (dependency.dependency_id, dependency.source_entity_id, dependency.target_entity_id, dependency.meeting_id, dependency.relationship_type.value, _dump(dependency)),
            )
